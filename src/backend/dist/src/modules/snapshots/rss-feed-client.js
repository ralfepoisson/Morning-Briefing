...
export class HttpRssFeedClient {
    async fetchFeed(url) {
        let response;
        try {
...
function parseFeedDocument(xml, fallbackUrl) {
...
function parseRssFeed(xml, fallbackUrl) {
    ...
    const items = matchAll(channelBlock, /<item\b[^>]*>([\s\S]*?)<\/item>/gi).map(function mapItem(itemBlock) {
        ...
        const summary = cleanText(readTagValue(itemBlock, 'description') ||
            readTagValue(itemBlock, 'content:encoded'));
        const publishedAt = normalizeDateString(cleanText(readTagValue(itemBlock, 'pubDate')) ||
...
function parseAtomFeed(xml, fallbackUrl) {
    ...
    const items = matchAll(xml, /<entry\b[^>]*>([\s\S]*?)<\/entry>/gi).map(function mapEntry(entryBlock) {
        ...
        const summary = cleanText(readTagValue(entryBlock, 'summary') ||
            readTagValue(entryBlock, 'content'));
        const publishedAt = normalizeDateString(cleanText(readTagValue(entryBlock, 'published')) ||
...
function readAttribute(attrs, attributeName) {
