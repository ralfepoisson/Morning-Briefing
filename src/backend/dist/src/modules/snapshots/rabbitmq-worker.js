...
export async function handleRabbitMqDelivery(channel, message, processor, config) {
    ...
    try {
        ...
        await publishConfirmed(channel, config.exchange, routingKey, message, destination === 'retry' ? retryCount + 1 : retryCount);
        channel.ack(message);
    }
    catch (error) {
...
async function publishConfirmed(channel, exchange, routingKey, message, retryCount) {
    const messageId = typeof message.properties.messageId === 'string'
        ? message.properties.messageId
        : `delivery-${message.fields.deliveryTag}-${Date.now()}`;
    let returned = false;
    const handleReturn = (returnedMessage) => {
...
export async function runRabbitMqWorker(processor, env = process.env, signal = new AbortController().signal) {
...
async function runConnectedWorker(processor, config, signal) {
    ...
    try {
        ...
        const consumer = await channel.consume(config.queue, (message) => {
            if (!message) {
                return;
            }
...
function forwardOptions(properties, retryCount) {
    return {
        persistent: true,
        mandatory: true,
...
function getRetryCount(message, retryQueue) {
...
async function markWorkerReady(file) {
    await writeFile(file, `${new Date().toISOString()}\n`, { mode: 0o600 });
    await chmod(file, 0o600);
}
async function removeHealthFile(file) {
...
async function waitForDisconnect(model, channel, signal) {
...
async function abortableDelay(durationMs, signal) {
