...
import {
  getGmailOAuthClientFromEnvironment,
...
function createConnectionRouteDependencies(): ConnectionRouteDependencies {
  ...
  let gmailOAuthClient: ConnectionRouteDependencies['gmailOAuthClient'];
...
async function buildGmailAuthorizationUrl(
  request: Parameters<ConnectionRouteDependencies['defaultUserService']['getDefaultUser']>[0],
  input: {
    returnTo?: string;
    connectionId?: string;
