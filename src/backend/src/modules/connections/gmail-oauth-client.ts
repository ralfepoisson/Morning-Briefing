...
export class GmailOAuthClient {
  ...
  createAuthorizationUrl(state: GmailOAuthState): string {
  ...
  async exchangeAuthorizationCode(code: string): Promise<GmailTokenSet> {
  ...
  async refreshAccessToken(refreshToken: string): Promise<Pick<GmailTokenSet, 'accessToken' | 'expiresAt' | 'scope' | 'tokenType'>> {
    let response: Response;
  ...
  async getAccount(accessToken: string): Promise<GmailAccount> {
    let response: Response;
  ...
  signState(state: GmailOAuthState): string {
  ...
  verifyState(serialized: string): GmailOAuthState {
...
export function getGmailOAuthClientFromEnvironment(): GmailOAuthClient {
...
export function getDefaultGmailOAuthRedirectUri(frontendBaseUrl?: string): string {
