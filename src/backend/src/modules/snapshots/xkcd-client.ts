...
export class XkcdClientImpl implements XkcdClient {
  ...
  private async getLatestComicFromJson(): Promise<XkcdComic> {
  ...
  private async fetchLatestComicFromJson(): Promise<XkcdComic> {
  ...
  private async getLatestComicFromHomepage(): Promise<XkcdComic> {
    let response: Response;
...
function parseComicFromHomepage(html: string): XkcdComic {
...
function shouldRetryPrimaryRequest(error: Error, attempt: number): boolean {
...
function extractComicId(value: string): number {
  const parsed = Number.parseInt(value, 10);
  return Number.isInteger(parsed) && parsed > 0 ? parsed : 0;
}
