...
export async function connectRabbitMq(config: MessageBrokerConfig): Promise<ChannelModel> {
  if (!config.url) {
...
export async function checkRabbitMqReadiness(env: NodeJS.ProcessEnv = process.env): Promise<void> {
...
export async function readRabbitMqQueueStats(
  env: NodeJS.ProcessEnv = process.env
): Promise<{ readyMessages: number; retryMessages: number; deadLetterMessages: number; consumerCount: number }> {
  const config = getMessageBrokerConfig(env);
