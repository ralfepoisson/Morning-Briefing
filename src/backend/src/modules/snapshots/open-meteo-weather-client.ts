...
export interface WeatherClient {
  getSnapshot(input: WeatherSnapshotInput & { locationLabel: string }): Promise<WeatherSnapshotData>;
}

export class OpenMeteoWeatherClient implements WeatherClient {
  async getSnapshot(input: WeatherSnapshotInput & { locationLabel: string }): Promise<WeatherSnapshotData> {
...
function formatUvIndex(value: number): string {
  if (value < 3) {
    return 'Low';
  }
