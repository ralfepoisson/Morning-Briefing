...
import type {
  DashboardBriefingAggregationRepository,
...
export class DashboardBriefingAggregationService {
  constructor(private readonly repository: DashboardBriefingAggregationRepository) {}
  ...
  async buildInput(
    ...
    includedWidgetTypes: string[];
    skippedWidgets: Array<{ widgetId: string; reason: string }>;
  } | null> {
    ...
    const skippedWidgets: Array<{ widgetId: string; reason: string }> = [];
    const sections = dashboard.widgets.reduce<DashboardBriefingInput['sections']>((items, widget) => {
...
function buildSection(
  ...
  snapshot: DashboardBriefingWidgetSnapshotSource
): DashboardBriefingInput['sections'][number] | null {
...
function readLabelValueList(value: unknown): Array<{ label: string; value: string }> {
