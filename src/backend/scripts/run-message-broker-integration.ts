...
import {
  ...
  checkRabbitMqReadiness,
  connectRabbitMq
} from '../src/modules/snapshots/rabbitmq-connection.js';
...
import type {
  GenerateWidgetSnapshotEnvelope,
  GenerateWidgetSnapshotRequested
} from '../src/modules/snapshots/snapshot-job-types.js';
...
async function prepareDurability(): Promise<void> {
  await seedFixture();
...
async function verifyDurability(): Promise<void> {
  const { connection, channel } = await openChannel();
  try {
...
async function verifyDuplicateIdempotency(): Promise<void> {
  const { connection, channel } = await openChannel();
  try {
...
async function verifyUnackedRedelivery(): Promise<void> {
...
async function publishScenario(name: string): Promise<void> {
  const { connection, channel } = await openChannel();
  try {
...
async function handleOne(): Promise<void> {
  const { connection, channel } = await openChannel();
  try {
...
async function recoverScenario(name: string): Promise<void> {
  await handleOne();
  await assertCompleted(name, 1, 0);
  console.log(`ok - ${name} recovered after a real PostgreSQL connectivity failure`);
}

async function assertDeadLetter(name: string): Promise<void> {
  const { connection, channel } = await openChannel();
  try {
...
async function verifyMalformedDeadLetter(): Promise<void> {
  const { connection, channel } = await openChannel();
  try {
    ...
    await handleRabbitMqDelivery(
      channel,
      await waitForMessage(channel, config.queue),
      createQueueJobProcessor(),
      deliveryConfig()
    );
...
async function seedScheduledProducers(): Promise<void> {
  await seedFixture();
  const prisma = getPrismaClient();
...
function envelopeFor(name: string): GenerateWidgetSnapshotEnvelope {
...
async function openChannel(): Promise<{ connection: Awaited<ReturnType<typeof connectRabbitMq>>; channel: ConfirmChannel }> {
  const connection = await connectRabbitMq(config);
...
async function publish(channel: ConfirmChannel, envelope: unknown, messageId?: string): Promise<void> {
  const id = messageId || (envelope as GenerateWidgetSnapshotEnvelope).payload.jobId;
...
async function waitForMessage(channel: ConfirmChannel, queue: string, timeoutMs = 10_000): Promise<Message> {
...
function toProcessorMessage(message: Message) {
  return {
...
function parseScenario(message: Message): string | undefined {
  const body = JSON.parse(message.content.toString('utf8')) as {
    payload?: { jobId?: string; scenario?: string };
  };
...
async function assertCompleted(name: string, attemptCount: number, duplicateSkipCount: number): Promise<void> {
  const prisma = getPrismaClient();
